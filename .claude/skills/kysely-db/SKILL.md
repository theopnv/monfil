...
Don't hand-write `.where()` chains for new queries — extend the relevant `CriteriaHandlers` object in `src/main/db/query.ts` and call `applyCriteria`. See that file for the existing pattern before adding a new query function next to it.

## Writing: upsert with `onConflict`

The insert helpers in `src/main/db/insert.ts` follow one shape — reuse it rather than inventing a new one:
...
- Pick the execute method by what "no row" means: `executeTakeFirstOrThrow()` when a missing row is a bug (wrap the call in `try/catch` and map to the `Result` type from `src/utils.ts`); `executeTakeFirst()` when a missing row is a valid outcome; `execute()` for anything that returns a set.
...
Look at `src/main/db/insert.test.ts` or `src/main/db/query.test.ts` for the current shape before writing a new test file.
