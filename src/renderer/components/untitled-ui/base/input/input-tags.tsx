...
import { Tag, TagGroup, TagList } from "@/components/untitled-ui/base/tags/tags";
...
export const InputTags = ({
    ...
    const entries = (() => {
        ...
        for (const label of value) {
            ...
            if (oldIndex !== -1) {
                ...
                newEntries.push(oldEntries[oldIndex] as TagEntry);
            } else {
    ...
    const focusLastTag = useCallback(() => {
        ...
        if (tagEls && tagEls.length > 0) {
            (tagEls[tagEls.length - 1] as HTMLElement).focus();
        }
    }, []);
    ...
    const handleTagGroupKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
        if (event.key === "ArrowRight") {
            const tagEls = tagGroupRef.current?.querySelectorAll<HTMLElement>('[role="row"]');
            if (tagEls && tagEls.length > 0) {
                const lastTag = tagEls[tagEls.length - 1] as HTMLElement;
