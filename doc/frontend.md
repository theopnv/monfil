...
`src/renderer/routeTree.gen.ts` is generated by the TanStack router Vite plugin. Do not edit it. It regenerates on the next dev run or build.
...
The window uses `titleBarStyle: 'hidden'`. `AppShell` draws the strip that replaces the title bar, and that strip carries the drag region. An element inside a drag region does not receive clicks unless it opts back out.
